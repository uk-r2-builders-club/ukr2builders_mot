jQuery(document).ready(function(){
	...
	jQuery('#change-pic').on('click', function(e){
        jQuery('#pic_upload_modal').modal({show:true});        
    });	
	jQuery('#pic').on('change', function()	{ 
		jQuery("#preview-pic").html('');
		jQuery("#preview-pic").html('Uploading....');
			...
			data: {
				...
				id: jQuery('#hdn-profile-id').val(),
				 t: 'ajax',
									...
									x2:params['x2_axis'],
									y2:params['y2_axis'],
									member_uid:jQuery('#member_uid').val(),
									image_name :jQuery('#image_name').val()
			},
			type: 'Post',
		   	success: function (response) {
					jQuery('#pic_upload_modal').modal('hide');
					jQuery(".imgareaselect-border1,.imgareaselect-border2,.imgareaselect-border3,.imgareaselect-border4,.imgareaselect-border2,.imgareaselect-outer").css('display', 'none');
					
					jQuery("#picture").attr('src', response);
					jQuery("#preview-pic").html('');
					jQuery("#pic").val();
			},
